episode "klustered-newcomers-1" {
  title = "Newcomers #1"
  draft = false
  show = "klustered"
  live = true
  scheduled_for = "2021-05-27T16:00:00Z"
  youtube_id = "H2227nrkhOg"
  youtube_category = 24
  links = [
    "https://youtu.be/teB22ZuV_z8",
    "https://youtu.be/JzGv36Pcq3g",
    "https://youtu.be/Ps2CQm6_aZU",
    "https://youtu.be/Cp6zvBIo5KM",
    "https://youtu.be/Ju1WmHfK6t8",
    "https://youtu.be/tmsqYWBTxEQ",
    "https://youtu.be/Pd90XGptVec",
    "https://youtu.be/QFxJWPF-QDk",
    "https://youtu.be/1f6KTDo5WEo",
    "https://youtu.be/RGaUhqgrsXE",
    "https://youtu.be/K72fOdbxXu8",
    "https://youtu.be/ysfUgYs4YYY",
    "https://youtu.be/akJCvD0ASmw",
    "https://youtu.be/5Rw7_1Yvm0U",
    "https://youtu.be/4lMxlQ64Z7I",
    "https://youtu.be/ozbE25Y_rcM",
    "https://youtu.be/IWz1XJnOR_g",
    "https://youtu.be/_BFbrrXKMOM",
    "https://youtu.be/JvgpDTx1AZ0",
    "https://youtu.be/_GZ8UuEZxbY",
    "https://youtu.be/_BFbrrXKMOM",
    "https://youtu.be/z0Lf303tKtQ",
    "https://youtu.be/-k5y2C6HNa0",
    "https://youtu.be/FClIbQ8hdxY",
    "https://twitter.com/rawkode",
    "https://rawkode.live/",
    "https://rawkode.live/chat",
    "https://twitter.com/thomcrowe",
    "https://github.com/thomcrowe",
    "https://twitter.com/penguin",
    "https://github.com/jeremytanner",
    "https://kubernetes.io/",
    "https://twitter.com/kubernetesio",
    "https://github.com/kubernetes/kubernetes"
  ]

  chapter "Viewer Comments" {
    time = "00:00:00"
  }

  chapter "Introductions" {
    time = "00:00:50"
  }

  chapter "kubeadm KUBECONFIG" {
    time = "00:05:00"
  }

  chapter "kubectl get pods" {
    time = "00:08:00"
  }

  chapter "kubectl describe pods" {
    time = "00:08:30"
  }

  chapter "Containerd logs" {
    time = "00:16:50"
  }

  chapter "Kubelet logs" {
    time = "00:18:20"
  }

  chapter "kubectl describe deployment" {
    time = "00:19:45"
  }

  chapter "kubectl edit deployment" {
    time = "00:23:20"
  }

  chapter "Pod Requests & Limits" {
    time = "00:28:00"
  }

  chapter "Liveness & Readiness Probes" {
    time = "00:35:00"
  }

  chapter "Static Pod Manifests" {
    time = "00:39:00"
  }

  chapter "Debugging Kubernetes Services" {
    time = "00:43:00"
  }

  chapter "kubectl scale" {
    time = "00:47:00"
  }

  chapter "ImagePullPolicies" {
    time = "00:54:30"
  }

  chapter "Service Endpoints" {
    time = "00:00:00"
  }
}
