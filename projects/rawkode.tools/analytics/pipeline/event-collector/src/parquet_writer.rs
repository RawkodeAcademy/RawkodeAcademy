...
impl ParquetWriter {
    ...
    pub async fn write_events_to_r2(&self, events: &[Event], event_type: &str) -> Result<()> {
        ...
        log_info(&format!(
            "Preparing to write {} events to R2 at: {}",
            events.len(),
            key
        ));
        ...
        log_info(&format!(
            "Converting {} events to JSON strings",
            events.len()
        ));
        ...
        log_info(&format!(
            "Successfully converted {} events to JSON",
            json_strings.len()
        ));
        ...
        match bucket.put(&key, parquet_buffer).execute().await {
            ...
            Err(e) => {
                log_error(&format!(
                    "Failed to write to R2 bucket: {}. Error: {}",
                    key, e
                ));
                Err(e)
            }
    ...
    pub async fn write_events_to_analytics_engine(&self, events: &[Event]) -> Result<()> {
        ...
        log_info(&format!(
            "Analytics Engine writes are not yet implemented. Would write {} events",
            events.len()
        ));
