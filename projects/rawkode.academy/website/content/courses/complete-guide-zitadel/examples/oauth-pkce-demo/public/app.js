...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (code) {
        try {
            ...
            if (!wellKnownUrl) {
                wellKnownUrl = prompt('Please enter the OpenID well-known configuration URL:');
                if (wellKnownUrl) {
    ...
    function displayTokens(tokens) {
        ...
        if (tokens.id_token) {
            ...
            if (claims) {
                ...
                // Check for any claim containing 'role' in the key
                    ...
                    for (const [key, value] of Object.entries(claims)) {
                        if (key.toLowerCase().includes('role') && (Array.isArray(value) || typeof value === 'object')) {
                ...
                document.getElementById('user-roles').textContent = Array.isArray(roles) && roles.length > 0
                    ? roles.join('\n') 
            ...
            document.getElementById('token-claims').textContent = 'Unable to decode claims';
