      ...
      validate:
        cel:
          ...
          - expression: |
              ...
              object.metadata.labels.team in params.data &&
              object.spec.replicas <= int(params.data[object.metadata.labels.team])
        ...
        - imageReferences:
          ...
          validate:
            cel:
              expressions:
              - expression: |
                  attestations.all(att, 
                    att.predicateType == "https://slsa.dev/provenance/v0.2" &&
                    att.predicate.buildType == "https://tekton.dev/chains/v2"
                  )
                message: "Images must be built using Tekton with SLSA provenance"
              - expression: |
                  attestations.exists(att,
                    att.predicateType == "https://spdx.dev/Document" &&
                    att.predicate.packages.all(pkg, 
                      !pkg.versionInfo.contains("SNAPSHOT") &&
                      !pkg.versionInfo.contains("alpha")
                    )
                  )
                message: "Images cannot contain snapshot or alpha dependencies"
```
