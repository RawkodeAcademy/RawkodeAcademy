...
const execute = async (client: Client): Promise<void> => {
	await task(command.name, async ({ task, setStatus }) => {
		...
		switch (pullRequest.isIt) {
			case true:
				setStatus("Pull Request / Preview Deploy");
				break;
			case false:
				setStatus("Main Branch");
				break;
		}
		...
		if (pullRequest.isIt) {
			await task(
				...
				async ({ setError, setOutput, setStatus }) => {
					...
					if (
						!pullRequestNumberMatcher ||
						pullRequestNumberMatcher.length < 2
					) {
						return setError(
