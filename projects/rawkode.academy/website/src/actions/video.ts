...
export const trackVideoEvent = defineAction({
  ...
  handler: async (event, _ctx) => {
    try {
      ...
      const influxDBToken = getSecret("INFLUXDB_API_TOKEN");
      const influxDBUrl = getSecret("INFLUXDB_API_URL");

      console.log(influxDBUrl);
      console.log(influxDBToken);

      // Not configured, that's OK
      if (!influxDBToken || !influxDBUrl || !INFLUXDB_ORG) {
