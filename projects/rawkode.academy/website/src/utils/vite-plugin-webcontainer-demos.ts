      ...
      try {
        ...
        for (const example of examples) {
          ...
          try {
            ...
            config = {
              title: example.name.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
  ...
  return {
    ...
    async load(id) {
      ...
      demos.forEach((demo, index) => {
        ...
        imports.push(`const demo${index} = import.meta.glob('${globPattern}', { as: 'raw', eager: true });`);
        demoMap.push(`'${key}': { files: demo${index}, config: ${JSON.stringify(demo.config)}, path: '${demo.path}' }`);
      });
