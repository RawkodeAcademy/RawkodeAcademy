...
function createNavigationItem(
	...
	switch (category) {
		case "Videos":
			description =
				segments.length > 1 ? "Watch this video" : "Browse all videos";
			break;
		case "Articles":
			description =
				segments.length > 1 ? "Read this article" : "Browse all articles";
			break;
		case "Technology":
			description =
				segments.length > 1 ? `Learn about ${title}` : "Browse technologies";
			break;
		case "Series":
			description =
				segments.length > 1 ? `Watch ${title} series` : "Browse video series";
			break;
		case "Organizations":
			description =
				segments.length > 1 ? `Learn about ${title}` : "Partner organizations";
			break;
		case "Documentation":
