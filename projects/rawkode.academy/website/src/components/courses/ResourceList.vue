...
const openEmbedModal = (resource: Resource) => {
  if (resource.embedConfig?.container === 'webcontainer') {
    ...
    const courseId = courseIndex >= 0 ? pathParts[courseIndex + 1] : props.courseId || 'unknown';
    
    const url = `/embed/webcontainer?course=${courseId}&resource=${resourceId}`;
    window.open(url, 'webcontainer', 'width=1200,height=800,toolbar=no,menubar=no,location=no,status=no,noopener,noreferrer');
  } else {
