...
export async function renderAndSanitizeArticle(
  ...
  try {
    ...
    try {
      ...
      htmlContent +=
        `<hr/><p><em>Note: Full article content with interactive elements is available on our website.</em></p>`;
      htmlContent +=
        `<p><a href="/read/${article.id}/">Read the full article with all features on our website</a></p>`;
    } catch (renderError) {
      console.warn(
