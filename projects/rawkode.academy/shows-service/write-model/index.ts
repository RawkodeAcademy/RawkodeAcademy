	...
	handlers: {
		create: async (ctx: Context, show: T) => {
			...
			const client = createClient({
				url: Deno.env.get("LIBSQL_URL") as string,
				authToken: Deno.env.get("LIBSQL_TOKEN") as string,
			});
			...
			const checks = await db.select().from(showsTable).where(eq(showsTable.id, id));

			if (checks.length > 0) {
				throw new TerminalError(`New show, ${show.name}, does not pass unique ID constraints. ID was generated as ${id}.`, { errorCode: 400 });
			}
...
const handler = endpoint().bind(showsService).withIdentityV1(Deno.env.get("RESTATE_IDENTITY_KEY") as string).bidirectional().handler();
