...
podman image build --target=read-model --tag europe-west2-docker.pkg.dev/rawkode-academy-production/rawkode-academy/chapters-read:${epoch} .
podman image push europe-west2-docker.pkg.dev/rawkode-academy-production/rawkode-academy/chapters-read:${epoch}

gcloud run deploy chapters-read \
      --image=europe-west2-docker.pkg.dev/rawkode-academy-production/rawkode-academy/chapters-read:${epoch} \
      --region=europe-west2 \
      --use-http2 \
...
podman image build --target=write-model --tag europe-west2-docker.pkg.dev/rawkode-academy-production/rawkode-academy/chapters-write:${epoch} .
podman image push europe-west2-docker.pkg.dev/rawkode-academy-production/rawkode-academy/chapters-write:${epoch}

gcloud run deploy chapters-write \
      --image=europe-west2-docker.pkg.dev/rawkode-academy-production/rawkode-academy/chapters-write:${epoch} \
      --tag v${epoch} \
      --region=europe-west2 \
      --use-http2 \
      --allow-unauthenticated \
...
deno run -A --no-config 'npm:@restatedev/restate' deployments register https://v${epoch}---chapters-write-wlnfqm3bkq-nw.a.run.app
```
