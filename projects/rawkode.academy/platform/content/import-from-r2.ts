...
import { contentTable } from './data-model/schema.ts';
...
const isEpisode = (title: string, description: string): boolean => {
	if(title.includes('Rawkode Live')) {
		return true;
	}

	if (title.includes('Compass') || description.includes('Cloud Native Compass')) {
		return true;
	}

	if (title.includes('Klustered')) {
		return true;
	}
...
for (const directory of allDirectories.directories) {
	...
	await db
		...
		.values({
			id: dirName,
			contentType: isEpisode(metadata.title, metadata.description) ? 'episode' : 'video',
			title: cleanTitle,
			subtitle: '',
