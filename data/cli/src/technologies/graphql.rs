...
pub async fn all_shows(ctx: &Context<'_>, filters: Option<TechnologyFilters>) -> Result<Vec<Show>> {
    let technologies = ctx.data_unchecked::<SqlitePool>();
    ...
    match q.fetch_all(technologies).await {
        Ok(shows) => Ok(shows),
